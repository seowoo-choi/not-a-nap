...
# 리듬·일지·엔딩 정보 계약 (2026-07-27)

- 다음 밤 SETUP은 밤 역할 문장과 `ReflectionResolver.GetRhythms()` 결과를 표시한다. 둘째 밤은 최대 1개,
...
- V2 아기띠 버클 고장의 `CarrierDisabledTurns=2`는 사건 발동 뒤 수락된 시간 소비 돌봄
  행동 두 번에 걸쳐 감소한다. 사건을 발동시킨 행동 자체는 첫 차감에 포함하지 않는다.
- 가로·세로 일지는 알아차린 신호 → 실제 행동에서 배운 점 → 보호자의 변화 →
  형성된 리듬 → 가족의 준비 이해 → 공유 카드의 동일한 데이터 필드를 사용한다.
