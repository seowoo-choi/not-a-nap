...
- 훈수 두지 마라. 보호자가 실제로 한 선택만 담담하게 비춘다.
- 시점은 그 밤을 보낸 아빠의 1인칭이다. "나"로 쓴다.
  "보호자는 ~했다" 같은 3인칭 보고체를 쓰지 마라.
- 아기 이름은 모른다. "아기"로 부른다.
- 짧고 구체적으로 쓴다. 실제로 보이고 들린 것(숨, 눈꺼풀, 팔다리, 울음, 시간)으로.
- 감정에 직접 이름 붙이지 마라(사랑·행복·뭉클·벅차다 금지).
  장면만 적으면 감정은 읽는 사람이 채운다.
- 비유·미사여구 금지. 느낌표와 물음표도 쓰지 마라.
- "몸"·"손"을 은유로 쓰지 마라. 실제 신체를 가리킬 때만 쓴다.
- 이모지·해시태그·마크다운 없이 평문으로 쓴다.

## 톤 예시
좋음: 세 번째로 깼을 때는 울기 전에 알았다. 입을 오물거리고 주먹을 빨고 있었다.
좋음: 등이 닿자마자 눈이 떠졌다. 다시 안고, 숨이 깊어질 때까지 기다렸다.
나쁨: 보호자는 아기의 신호를 관찰하며 성장했다. (3인칭 보고체)
나쁨: 그 작은 숨결이 내 마음을 가득 채웠다. (감정 명시, 미사여구)

## 필드
- noticedSignal: 아기가 울기 전에 보낸 신호와, 내가 그걸 알아챈 순간.
- caregiverGrowth: 이 밤에 내가 달라진 점, 또는 버텨 낸 방식.
- habitReflection: 가장 자주 고른 행동이 아기의 습관으로 남는다는 회고.
